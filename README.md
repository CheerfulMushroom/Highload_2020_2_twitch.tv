...
В пиковом случае (все смотрят разные стримы) скачиваться контент из архива будут со скоростью  
...
#### Требуемые ресурсы  
Из расчётов следует, что нам нужны столько серверов, чтобы обеспечить приём и выдачу данных со скоростью **4700 Гбит/с** и суммарным объёмом в 157 ПБ. Примем, что наибольшая скорость интернета для одного сервера **20 Гб/c**, тогда нам понадобится 235 серверов минимум. Возьмём для запаса **280** серверов (20% коэффициент запаса).  
В таком случае, каждый из них должен иметь объём 157 ПБ / 280 = **560 ТБ**.  
Установим в каждый из серверов **по 30 дисков на 20 Тб**




### Edge-сервера  
Через edge сервера будет производиться распространения записи из архивов и live контента от транскодеров ко зрителям.  
...
Средняя скорость скачивания транскодированного видео = **1,07МБ/с**  
В пиковом случае загружаться видео в архив будут со скоростью  
185e3 \* 1,07 = 197950 МБ/с = **1600 Гбит/с**,  
где 185e3 - пиковое количесвто одновременных стримов  

#### Получение видео с архива
Как уже было расчитано, в пиковом случае из архива будет скачиваться **3600 Гбит/с**

### Выдача видео зрителям
Средняя скорость скачивания зрителем видео = **0,43 МБ/с**  
Всего зрителей 3.5e6
Пиковая скорость скачивания видео зрителями = 3.5e6 * 0.43 = **12000 Гбит/с**

#### Требуемые ресурсы  
Из расчётов следует, что нам нужны столько серверов, чтобы обеспечить приём и выдачу данных со скоростью **17200 Гбит/с**. Примем, что наибольшая скорость интернета для одного сервера **50 Гбит/c** (2 сетевые карты по 25Гбит/с), тогда нам понадобится 344 серверов минимум. Возьмём для запаса **410** серверов (20% коэффициент запаса).  
Установим в каждый из серверов **по 64GB RAM и 1 SSD на 120GB для системы**


### Итого  
| Роль  | CPU     | RAM    | SSD     | HDD     | Количество | Сетевые карты |
|-------|---------|--------|---------|---------|------------|---------------|
| БД    | 32 ядра | 128 Гб | 1x240GB | -       | 3          | 1*10 Gbit/s   |
| Архив | 32 ядра | 256 Гб | -       | 30x20TB | 235        | 2*10 Gbit/s   |
| Edge  | 64 ядра | 64 Гб  | 1x120GB | -       | 410        | 2*25 Gbit/s   |
...
### Трафик с архивных серверов у вас считается общий без учета тоо что он расщепляется на edge, придется пересчиттать
Скорость чтения из архивая считал в худшем случае, когда каждый зритель хочет смотреть разные записи стримов. Пересчитал Edge учитывая, что ему передаётся трафик из архива. 

### На Edge можете брать 2 сетевухи по 10 Гбит/с или даже по 25 Гбит/с они потянут
Пересчитал Edge

### Как будет выглядеть балансировка на edge когда в одной локации их стоит больше одного (например 10 или 20 в крупном городе)?
В route 53 можно настроить балансировку между конечными точками в регионе. Достаточно будет добавить round robin для того, чтобы размазать нагрузку между edge-серверами.

### Не описано резервирование Edge серверов (например пришел запрос из DNS на нерабочий сервер)
В route53 также можно настроить health check конечных точек. При обнаружении упавшего сервера, он перестанет направлять на него пользователей

### Не раскрыто что произойдет если в один edge-сервер через route53 придет больше пользовтелей чем он может обслужить
Такой ситуации произойти не должно, так как route 53 распределяет нагрузку между edge внутри региона с помощью round-robin. Такая ситуация произойдёт только если упадёт наш запас на 20% серверов, который мы учитывали при расчётах. 