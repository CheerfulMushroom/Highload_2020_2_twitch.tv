...
Для хранения информации будем использовать **16 SSD по 4 ТБ в каждом сервере**
...
В таком случае, каждый из них должен иметь объём 157 ПБ / 550 = **285 ГБ**.  
Установим в каждый из серверов **по 16 дисков на 20 Гб**
...
Для хранения такого количества видео, будем использовать по **20 SSD дисков по 2 ТБ**
...
#### Требуемые ресурсы  
Из расчётов следует, что нам нужны столько серверов, чтобы обеспечить приём и выдачу данных со скоростью **15600 Гбит/с**. Примем, что наибольшая скорость интернета для одного сервера **10 Гб/c**, тогда нам понадобится 1560 серверов минимум. Возьмём для запаса **1900** серверов (20% коэффициент запаса).  
Установим в каждый из серверов **по 20 SSD дисков на 2 Гб**


### Итого  
| Роль  | CPU     | RAM    | SSD    | HDD     | Количество |
|-------|---------|--------|--------|---------|------------|
| БД    | 32 ядра | 128 Гб | 16x4TB | -       | 3          |
| Архив | 32 ядра | 256 Гб | -      | 16x20GB | 550        |
| Edge  | 64 ядра | 256 Гб | 20x2TB | -       | 1900       |
